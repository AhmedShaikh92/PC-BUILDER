...
function formatSpecs(specs: Record<string, unknown>): string {
  return Object.entries(specs)
    ...
    .map(([k, v]) => {
      // Format the key: convert camelCase to Title Case with spaces
      const formattedKey = k
        .replace(/([A-Z])/g, ' $1') // Insert space before capital letters
        .replace(/^./, str => str.toUpperCase()) // Capitalize first letter
        .replace(/([a-z])([0-9])/g, '$1 $2') // Add space between letters and numbers
        .replace(/MMB/g, 'MM B') // Special case for MMB
        .trim();
...
/** Formats compact specs for the selected component view */
...
export function ComponentSelector({
  ...
  // ── Fetch prices whenever the selected component changes ───────────────
  useEffect(() => {
  ...
  // ── Close price details whenever the panel opens ───────────────────────
  useEffect(() => {
    if (isOpen) setShowPriceDetails(false);
  }, [isOpen]);
  ...
  // ── Derived ────────────────────────────────────────────────────────────
  const lowestVendorPrice = selectedComponentPrices.length
    ? Math.min(...selectedComponentPrices.map((p) => p.price))
    : null;
          ...
          <div className="flex items-start justify-between gap-3 px-3 py-2.5">
            <div className="min-w-0 flex-1">
              ...
              <p className="text-xs text-neutral-500 mt-0.5 truncate">
                {formatCompactSpecs(selectedComponent.component.specs as Record<string, unknown>)}
              </p>
            </div>
        ...
        <div className="mb-6 border border-neutral-800 overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-neutral-900 bg-neutral-950">
            {loading ? (
              ...
              components.map((comp) => {
                const isSelected =
                  selectedComponent?.component._id === comp._id;
                ...
                return (
                  ...
                  >
                    ...
                    {/* Specs — with keys and formatted values */}
                    {comp.specs && (
